...
class TestRateLimitInUpsert:
    ...
    @responses.activate
    def test_retries_on_429_insert(self, monkeypatch, sample_travelperk_user_payload):
        """Test retries insert on 429 response."""
        upserter = get_upserter_module(monkeypatch)

        # First GET returns empty list (user doesn't exist)
        responses.add(
            responses.GET,
    ...
    @responses.activate
    def test_retries_on_429_update(self, monkeypatch, sample_travelperk_user_payload, sample_travelperk_user_response):
        """Test retries update on 429 response."""
        upserter = get_upserter_module(monkeypatch)

        # GET returns existing user
        responses.add(
            responses.GET,
            re.compile(r".*/api/v2/scim/Users.*"),
            json={"Resources": [sample_travelperk_user_response]},
            status=200,
        )
        # First PATCH returns 429
        responses.add(
            responses.PATCH,
            re.compile(r".*/api/v2/scim/Users/.*"),
            status=429,
    ...
    @responses.activate
    def test_429_uses_retry_after_header(self, monkeypatch, sample_travelperk_user_payload):
        ...
        # First POST returns 429 with specific Retry-After
        responses.add(
            responses.POST,
            re.compile(r".*/api/v2/scim/Users$"),
            status=429,
        ...
        with patch("time.sleep") as mock_sleep:
            upserter.upsert_user_payload(sample_travelperk_user_payload)
...
class TestCorrelationId:
    ...
    @responses.activate
    def test_result_includes_correlation_id(self, monkeypatch, sample_travelperk_user_payload):
    ...
    @responses.activate
    def test_auto_generates_correlation_id(self, monkeypatch, sample_travelperk_user_payload):
    ...
    def test_dry_run_includes_correlation_id(self, monkeypatch, sample_travelperk_user_payload):
        """Test dry run includes correlation ID."""
        upserter = get_upserter_module(monkeypatch)

        result = upserter.upsert_user_payload(
            sample_travelperk_user_payload,
            dry_run=True,
            correlation_id="dry-run-cid"
        )

        assert result.get("correlation_id") == "dry-run-cid"
...
class TestGlobalRateLimiter:
    ...
    def test_api_calls_use_rate_limiter(self, monkeypatch):
        ...
        upserter.travelperk_get_user("tp-123")
